  ...
  return (
    <div className="max-w-4xl space-y-8 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-gray-200">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Configure your repeating hours in <span className="text-orange-600 font-semibold">{timezone}</span>.
          </p>
        </div>
        <button
          onClick={handleSave}
          disabled={saving}
          className="py-2.5 px-5 rounded-xl bg-orange-600 hover:bg-orange-700 text-white text-xs font-semibold shadow-xs shadow-orange-500/20 flex items-center gap-2 transition-all disabled:opacity-50 cursor-pointer self-start sm:self-auto"
        >
          {saving ? (
      ...
      <div className="p-6 rounded-2xl bg-white border border-gray-200 shadow-xs space-y-6">
        <h2 className="text-base font-bold text-gray-900">Recurring Weekly Schedule</h2>
            ...
            return (
              <div key={day.id} className="py-4 first:pt-0 last:pb-0 flex flex-col md:flex-row md:items-start justify-between gap-4">
                ...
                <div className="flex-1 space-y-2.5">
                  {!enabled ? (
                    <span className="text-xs text-gray-400 italic block pt-1">Unavailable / Day Off</span>
                  ) : (
                    dayRules.map((rule, idx) => (
                      <div key={idx} className="flex items-center gap-2">
                        <input
                          ...
                          className="px-3 py-1.5 rounded-lg bg-gray-50 border border-gray-200 text-xs text-gray-900 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20"
                        />
      ...
      <div className="p-6 rounded-2xl bg-white border border-gray-200 shadow-xs space-y-5">
        <div>
          <h2 className="text-base font-bold text-gray-900">Date Overrides & Blocked Days</h2>
        ...
        <div className="flex flex-wrap items-center gap-3 p-4 rounded-xl bg-gray-50 border border-gray-200">
          <input
            ...
            className="px-3 py-2 rounded-lg bg-white border border-gray-200 text-xs text-gray-900 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20"
          />
          <button
            type="button"
            onClick={handleAddOverride}
            disabled={!newOverrideDate}
            className="py-2 px-4 rounded-lg bg-gray-900 hover:bg-black disabled:opacity-40 text-white text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
          >
              ...
              {overrides.map((override) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono font-medium text-gray-800">{override.date}</span>
